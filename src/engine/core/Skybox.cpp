...
void Skybox::render(SceneState& state) {
...
Skybox::Skybox(const SkyboxTextures& skybox, const char* vPath, const char* fPath) {
  auto key = string(vPath) + "/" + string(fPath);
  skyboxShader = unique_ptr<SimpleShader>(new SimpleShader(vPath, fPath));
